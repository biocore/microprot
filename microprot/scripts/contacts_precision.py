...
add option to calculate HA contacts (need to re-assess the method to calculate distances)
when printing output make sure only filename is included (no path)
...
def FindPDBContacts(con_fp, pdb_fp, out_fp, aacid, coff, mind, topl, contype):
	...
	if foutput:
		foutput.close()
	print(str(pdb_fp) + " Top-L/" + str(topl) + " precision: " + str(calc_prec(true,false)))

def main():
	parser = OptionParser(usage="usage: %prog [OPTIONS] [psicov_file] [pdb] OPTIONAL:[output]")
	parser.add_option("-a", dest='aminoacid', default='CB', help="CA, CB etc. (Default: CB)")
	parser.add_option("-c", dest='cutoff', default='8', help="Cutoff (Default: 8)")
	parser.add_option("-d", dest='mindist', default='5', help="Minimum sequence separation (Default: 5)")
	parser.add_option("-l", dest='topL', default='1', help="Top-L/x contacts (Default: 1)")
	parser.add_option("-t", dest='conType', default='all', help="Type of contacts [all | sr | lr] (Default: all)")
